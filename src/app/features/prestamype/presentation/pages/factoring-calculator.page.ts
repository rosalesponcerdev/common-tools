      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div class="lg:col-span-1 space-y-6">
            <div class="bg-white dark:bg-zinc-800 rounded-2xl p-6 border border-zinc-200 dark:border-zinc-700">
              ...
              <div class="space-y-4">
                <div>
                  ...
                  @if (facade.filteredInfo() && facade.filteredInfo()!.count > 0) {
                    <p class="mt-2 text-xs text-amber-600 dark:text-amber-400">
                      ⚠️ {{ facade.filteredInfo()!.count }} facturas en {{ facade.filteredInfo()!.currency }} fueron filtradas (solo se procesan PEN)
                    </p>
                  }
                </div>
                ...
                <div>
                  ...
                  @if (!facade.isMontoValido() && facade.montoInversion() > 0) {
            ...
            @if (facade.comparacionResultados().length > 0) {
              <div class="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                ...
                <app-factoring-comparison-table
                  [resultados]="facade.comparacionResultados()"
                  [montoInversion]="facade.montoInversion()"
                />
              </div>
            } @else {
              <div class="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-12 text-center">
...
export class FactoringCalculatorPage {
  ...
  onJsonInput(event: Event): void {
    ...
    this.facade.updateJsonInput(target.value);
  }

  onMontoInput(event: Event): void {
    const target = event.target as HTMLInputElement;
    this.facade.updateMontoInversion(Number(target.value));
  }

  onTeaInput(event: Event): void {
    const target = event.target as HTMLInputElement;
    this.facade.updateTeaPlazoFijo(Number(target.value));
  }

  getTotalFactoring(): number {
    return this.facade.comparacionResultados().reduce(
      (sum, r) => sum + r.factoring.gananciaNeta,
      0
    );
  }
  ...
  getTotalPlazoFijo(): number {
    return this.facade.comparacionResultados().reduce(
      (sum, r) => sum + r.plazoFijo.gananciaNeta,
      0
    );
  }
