    ...
    >
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
          <h3 class="font-semibold text-lg text-zinc-900 dark:text-zinc-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
            {{ tool().name }}
          </h3>
      ...
      <div class="mt-auto pt-4 flex items-center justify-between">
        <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300">
          {{ tool().category }}
        </span>
        <span class="text-zinc-400 dark:text-zinc-500 group-hover:translate-x-1 transition-transform">
          →
        </span>
      </div>
    </a>
